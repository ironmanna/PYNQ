...
foreach mb $processors {
    # MicroBlaze instances may be hierarchical (e.g. iop_pmod0/mb); use the
    # hierarchy name for the BSP so the generated directory is filesystem-safe
    # and matches the per-IOP layout expected by build.mk.
    set mb_name [string map {/ _} $mb]
    if {[string match */* $mb]} {
        set mb_name [lindex [split $mb /] 0]
    }
    set bsp "bsp_${mb_name}"
    if {![file exists $bsp]} {
        puts "Creating new BSP ${bsp} for processor ${mb} ..."
        ...
        bsp regenerate
        file copy "[pwd]/lscript.ld" $bsp/${mb_name}/standalone_domain/bsp
    } else {
