...
class RemoteInterrupt:
    ...
    def _ensure_registered(self):
        ...
        try:
            ...
            if resp.interrupt_id:
                self._interrupt_id = resp.interrupt_id
            elif resp.msg:
                warnings.warn(f"Interrupt registration failed: {resp.msg}")
        except grpc.RpcError as e:
            warnings.warn(f"Interrupt registration RPC failed: {e}")
    ...
    async def wait(self):
        ...
        """
        if self._interrupt_id is None:
            raise RuntimeError("Interrupt not registered")
